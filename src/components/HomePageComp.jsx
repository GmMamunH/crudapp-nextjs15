...
const HomePageComp = ({ users }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-10/12 gap-5">
      <div className="bg-white w-full md:w-8/12 p-5 rounded-md flex flex-col gap-4 shadow-md">
        <div className="border-b-2">
          <h2 className="text-center text-xl font-semibold">
            {edit ? "Update User details" : "Create an User"}
          </h2>
        </div>
        <div className="flex flex-col gap-3">
          <div>
            <input
              ...
              className="outline-none border px-3 py-2 text-xl rounded-md w-full"
          ...
          <div>
            <button
              className="w-full bg-blue-500 text-white px-2 py-2 rounded-md cursor-pointer hover:bg-blue-700 duration-300"
              onClick={edit ? handleUpdate : handleSave}
            >
              {edit ? "Update" : "Save User"}
            </button>
          </div>
        </div>
      </div>
      <div className="bg-white w-full md:w-8/12 p-5 rounded-md shadow-md flex flex-col gap-4">
        <div className="border-b-2">
              ...
              <table className="min-w-full border-collapse border border-gray-200">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="px-4 py-2 text-left font-medium text-gray-600 border-b border-gray-300">
                      #
                    </th>
                    <th className="px-4 py-2 text-left font-medium text-gray-600 border-b border-gray-300">
                      Name
                    ...
                    users.map((user, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 border-b border-gray-200 flex gap-2">
                          <button
                            className="px-3 py-1 text-white bg-blue-500 rounded hover:bg-blue-600 cursor-pointer"
                            onClick={() => handleEditGetUesr(user._id)}
                          >
                            <FaEdit />
                          </button>
                          <button
